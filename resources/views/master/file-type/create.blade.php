...
@section('title')
    {{ __('Create') }} {{ __('File Type') }} | {{ config('app.name') }}
@endsection

@section('subheader')
    {{ __('Create') }} {{ __('File Type') }}
@endsection
...
@section('breadcrumb')
    ...
    <span class="kt-subheader__breadcrumbs-separator"></span><a href="{{ route('master.file-type.create') }}" class="kt-subheader__breadcrumbs-link">{{ __('Create') }} {{ __('File Type') }}</a>
@endsection

@section('content')
<form class="kt-form" id="kt_form_1" action="{{ route('master.file-type.store') }}" method="POST">
    @csrf

    <div class="kt-portlet" id="kt_page_portlet">
        <div class="kt-portlet__head kt-portlet__head--lg">
            <div class="kt-portlet__head-label">
                <h3 class="kt-portlet__head-title">{{ __('Create') }} {{ __('File Type') }}</h3>
            </div>
