...
@section('title')
    {{ __('Edit') }} {{ __('National Holiday') }} | {{ config('app.name') }}
@endsection
...
@section('breadcrumb')
    ...
    <span class="kt-subheader__breadcrumbs-separator"></span><a
        href="{{ route('master.national-holiday.edit', $nationalHoliday->id) }}"
        class="kt-subheader__breadcrumbs-link">{{ $nationalHoliday->name }}</a>
@endsection

@section('content')
    <form class="kt-form" id="kt_form_1" action="{{ route('master.national-holiday.update', $nationalHoliday->id) }}"
        method="POST">
        @method('PUT')
        @csrf
        ...
        <div class="kt-portlet" id="kt_page_portlet">
            <div class="kt-portlet__head kt-portlet__head--lg">
                <div class="kt-portlet__head-label">
                    <h3 class="kt-portlet__head-title">{{ __('Edit') }} {{ __('National Holiday') }}</h3>
                </div>
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                ...
                                <input id="name" name="name" type="text"
                                    ...
                                    value="{{ old('name', $nationalHoliday->name) }}" required autocomplete="off">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input id="date" name="date" type="text"
                                    ...
                                    value="{{ $nationalHoliday->date }}" required>
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <select class="form-control kt_selectpicker @error('religion') is-invalid @enderror"
                                    ...
                                    <option value="Muslim"
                                        {{ old('religion', $nationalHoliday->religion) == 'Muslim' ? 'selected' : '' }}>
                                        {{ __('Muslim') }}</option>
                                    ...
                                    <option value="Buddhism"
                                        {{ old('religion', $nationalHoliday->religion) == 'Buddhism' ? 'selected' : '' }}>
                                        {{ __('Buddhism') }}</option>
                                    <option value="Confucianism"
                                        {{ old('religion', $nationalHoliday->religion) == 'Confucianism' ? 'selected' : '' }}>
                                        {{ __('Confucianism') }}</option>
                                    <option value="-"
                                        {{ old('religion', $nationalHoliday->religion) == '-' ? 'selected' : '' }}>
                                        {{ __('-') }}</option>
                                </select>
