...
@section('title')
    {{ __('Create') }} {{ __('Role') }} | {{ config('app.name') }}
@endsection
...
@section('breadcrumb')
    <span class="kt-subheader__breadcrumbs-separator"></span><a href="{{ route('role.index') }}" class="kt-subheader__breadcrumbs-link">{{ __('Role') }}</a>
    <span class="kt-subheader__breadcrumbs-separator"></span><a href="{{ route('role.create') }}" class="kt-subheader__breadcrumbs-link">{{ __('Create') }} {{ __('Role') }}</a>
@endsection

@section('content')
<div class="row" data-sticky-container>
    <div class="col-lg-9">
        <form class="kt-form" id="kt_form_1" action="{{ route('role.store') }}" method="POST">
            @csrf

            <div class="kt-portlet" id="kt_page_portlet">
                <div class="kt-portlet__head kt-portlet__head--lg">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">{{ __('Create') }} {{ __('Role') }}</h3>
                    </div>
                    ...
                    <div class="kt-section kt-section--first">
                        <div class="kt-section__body">
                            ...
                            <div class="form-group form-group-last">
                                ...
                                <input id="nama" name="nama" type="text" class="form-control @error('nama') is-invalid @enderror" required placeholder="{{ __('role_name') }}" value="{{ old('nama') }}">
                                    ...
                                    @foreach ($permissions as $i => $permission)
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <span class="kt-switch kt-switch--sm kt-switch--icon kt-switch--primary kt-switch--outline">
                                                    <label class="mb-0">
                                                        <input type="checkbox" value="{{ $permission->id }}" data-group="{{ $group }}" name="hak_akses[{{ $group }}][{{ $i }}]" {{ old('hak_akses.' . $group . '.' . $i) == $permission->id ? 'checked' : '' }}>
                                                        <span class="m-0"></span>
                                                    </label>
                                                </span>
                                            </td>
                        ...
                        <ul class="kt-nav kt-nav--bold kt-nav--md-space kt-nav--v3 kt-margin-t-20 kt-margin-b-20 nav nav-tabs" role="tablist">
                            @foreach ($permissionGroups as $group => $permissions)
                                <li class="kt-nav__item">
                                    <a class="kt-nav__link active index" href="#{{ str_replace(' ' , '-', $group) }}" data-index="{{ $group }}">
