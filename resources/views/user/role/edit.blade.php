...
@section('breadcrumb')
    ...
    <span class="kt-subheader__breadcrumbs-separator"></span><a href="{{ route('role.show', $role->id) }}" class="kt-subheader__breadcrumbs-link">{{ $role->display_name }}</a>
@endsection
                    ...
                    <div class="kt-section kt-section--first">
                        <div class="kt-section__body">
                            ...
                            <div class="form-group form-group-last">
                                ...
                                <input id="nama" name="nama" type="text" class="form-control @error('nama') is-invalid @enderror" required placeholder="{{ __('role_name') }}" value="{{ old('nama', $role->name) }}">
                                    ...
                                    @foreach ($permissions as $i => $permission)
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <span class="kt-switch kt-switch--sm kt-switch--icon kt-switch--primary kt-switch--outline">
                                                    <label class="mb-0">
                                                        <input type="checkbox" value="{{ $permission->id }}" data-group="{{ $group }}" name="hak_akses[{{ $group }}][{{ $i }}]" {{ old('hak_akses.' . $group . '.' . $i) == $permission->id ? 'checked' : (in_array($permission->id, $rolePermissions) ? 'checked' : '') }}>
                                                        <span class="m-0"></span>
                                                    </label>
                                                </span>
                                            </td>
