...
@section('breadcrumb')
    ...
    <span class="kt-subheader__breadcrumbs-separator"></span><a href="{{ route('user.show', $user->id) }}" class="kt-subheader__breadcrumbs-link">{{ $user->email }}</a>
@endsection
                ...
                <div class="kt-section__body">
                    ...
                    <div class="row">
                        <div class="form-group col-sm-6">
                            ...
                            <input id="nama" name="nama" type="text" class="form-control @error('nama') is-invalid @enderror" required value="{{ old('nama', $user->name) }}">
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <select id="wewenang" name="wewenang" class="form-control kt_selectpicker @error('wewenang') is-invalid @enderror" required data-live-search="true" title="{{ __('Choose') }} {{ __('Role') }}">
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" {{ old('wewenang', $userRole) == $role->id ? 'selected' : '' }}>{{ $role->display_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <input id="nik" name="nik" type="text" class="form-control @error('nik') is-invalid @enderror" required value="{{ old('nik', $user->nik) }}">
                    ...
                    <div class="form-group">
                        ...
                        <span class="kt-switch kt-switch--icon kt-switch--primary kt-switch--outline">
                            <label class="mb-0">
                                <input id="aktif" name="aktif" type="checkbox" {{ old('aktif', $user->active) ? 'checked' : '' }}>
                                <span class="m-0"></span>
                            </label>
                        </span>
                    </div>
